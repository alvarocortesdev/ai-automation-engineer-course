...
aws s3 mb s3://donpelusa-app-uploads-prod

# Subir y listar
aws s3 cp ./factura.pdf s3://donpelusa-app-uploads-prod/facturas/factura.pdf
aws s3 ls s3://donpelusa-app-uploads-prod/facturas/
```
...
|---|---|---|
| **Policy** | Un documento JSON que dice **qué acciones** sobre **qué recursos** se permiten/niegan | "permitir `s3:GetObject` sobre `donpelusa-app-uploads-prod/*`" |
...
s3 = boto3.client("s3")
obj = s3.get_object(Bucket="donpelusa-app-uploads-prod", Key="facturas/factura.pdf")
