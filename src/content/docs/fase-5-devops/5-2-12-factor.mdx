...
- En `bitacora.md` explicas qué es config y qué no (con un ejemplo de cada uno de tu propio HomeBase o proyecto), y por qué un `.env` commiteado rompe el Factor III.
...
Escribe 3–4 frases respondiendo: **de tus proyectos actuales (HomeBase o cualquiera), ¿cuál tiene un secreto en el código, un estado en memoria o un log a archivo?** Ese es el primer factor que vas a querer pagar — y probablemente sea config o un secreto, los de mayor riesgo.
