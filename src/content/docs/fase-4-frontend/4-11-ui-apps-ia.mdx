...
> [!tip] En la práctica
> Acabas de aprender que la mitad del "magia" de un chat de IA no es el modelo: es una máquina de estados de cinco acciones y la cortesía de no dejar al usuario mirando una pantalla en blanco mientras algo "piensa". El `aria-live` no es para quien construye la app: es para quien la usa, sobre todo si no la ve. Lo pones igual. Eso es lo que llaman empatía de producto.
