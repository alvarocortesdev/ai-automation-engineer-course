...
> [!tip] En la práctica
> Acabas de aprender que tu trabajo en Next.js es, sobre todo, **decidir dónde corre cada línea**: servidor por defecto, navegador solo donde de verdad hace falta. Es casi filosófico —minimizar lo que mandas al cliente, igual que minimizar lo que mandas al mundo—. Pon `'use client'` en todo y tendrás una SPA disfrazada de cosa moderna, lenta y con la clave de API expuesta. Pon la frontera donde corresponde y tendrás algo que carga al instante y no te delata. Una de esas dos cosas la hace cualquiera con prisa. Apunta a la otra.
