...
> [!tip] En la práctica
> Imagina mil grabaciones de gente resolviendo problemas. ¿Cuáles valen? No las
> de quien hizo cien ejercicios fáciles. Las de quien resolvió **uno** difícil y dejó anotado cómo,
> qué intentó, qué falló y por qué la solución final era la correcta. Eso es lo que mira un reclutador:
> no cuántas cosas tocaste, sino la profundidad con la que cruzaste las que importan. Diez
> tutoriales clonados son ruido de fondo. Un capstone que corre, falla con elegancia y explica sus
> decisiones es una señal que se oye desde lejos.
...
la **plataforma RAG de F6** y **HomeBase** (fullstack con tests y CI/CD). Su instinto le dice "muestra
...
el reclutador ve en cada portafolio. El HomeBase es fullstack completo, buena prueba de amplitud. Pero el
...
**Decisión: el agéntico es la estrella (pin #1)**, el RAG segundo, HomeBase tercero.
...
| HomeBase (fullstack) | fullstack TS + CI/CD + estados completos (a11y) |
...
3. **¿Hay write-up de trade-offs?** Este es el que casi nadie tiene. Para el agéntico, Sofía escribe dos
   decisiones reales (lo veremos abajo). Para el RAG y HomeBase, una cada uno.
...
> [!info] Contexto
> "Mientras más proyectos, mejor" es mentira. Hay mil portafolios
> idénticos: la misma todo-app, el mismo chatbot sobre un PDF, el mismo silencio del
> reclutador. Diferenciarte no es hacer *más*. Es hacer *menos*, más profundo, y tener la decencia de
> explicar por qué tomaste cada decisión. Cura. Esconde el ruido. Deja que se vea el único proyecto
