...
**El contexto.** Tengo una app, _HomeHub_, que mi pareja y yo usamos de verdad
...
**El incidente.** Un sábado, mi pareja agrega tres productos a la lista desde su
...
```markdown
# Post-mortem: Silent data loss in HomeHub shopping list — 2026-06-13

**Status:** resolved · **Author:** @donpelusa · **Severity:** low (2 users, no PII)

## Summary
A deploy mid-request crashed an in-flight write. Two shopping-list items were
lost silently. The user (my partner) found out at the supermarket, not from an
...
|---|--------|-------|-----|--------|
| 1 | Add alert on write-path 5xx rate | @donpelusa | 2026-06-15 | done |
| 2 | Add a regression test that reproduces the lost write | @donpelusa | 2026-06-16 | done |
| 3 | Define an SLO: 99.5% of writes succeed | @donpelusa | 2026-06-20 | done |
